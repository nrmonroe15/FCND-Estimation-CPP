...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F accelRotated = attitude.Rotate_BtoI(accel);
  accelRotated.z = accelRotated.z - 9.81f;

  predictedState(0) += predictedState(3) * dt;
  predictedState(1) += predictedState(4) * dt;
  predictedState(2) += predictedState(5) * dt;

  predictedState(3) += accelRotated.x * dt;
  predictedState(4) += accelRotated.y * dt;
  predictedState(5) += accelRotated.z * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  MatrixXf ut(3, 1);  //convert accel from vector to matrix format
  ut(0, 0) = accel.x;
  ut(1, 0) = accel.y;
  ut(2, 0) = accel.z - 9.81f;

  MatrixXf matTotal = RbgPrime * ut * dt;

  gPrime(3, 6) = matTotal(0, 0);
  gPrime(4, 6) = matTotal(1, 0);
  gPrime(5, 6) = matTotal(2, 0);
